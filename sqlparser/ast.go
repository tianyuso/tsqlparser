...
import (
	...
	tidbparser "github.com/tianyuso/tsqlparser/tidbparser/parser"
)
...
type TableName struct {
	Name, Qualifier, Schema TableIdent
}
...
func (node TableName) Format(buf *TrackedBuffer) {
	if !node.Qualifier.IsEmpty() {
		if !node.Schema.IsEmpty() {
			...
			buf.Myprintf("%v.%v.%v", node.Qualifier, node.Schema, node.Name)
		} else {
			// database.table
			buf.Myprintf("%v.%v", node.Qualifier, node.Name)
		}
	} else {
		// just table
...
func (node TableName) String() string {
	if node.Schema.IsEmpty() {
		...
		return node.Qualifier.String() + "." + node.Name.String()
	}
	return node.Qualifier.String() + "." + node.Schema.String() + "." + node.Name.String()
}
...
// 新建一个结构体，用于存储with语句中的子查询
type With struct {
	Exprs CommonTableExprs
}
